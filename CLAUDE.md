...
- **Batch transcription (CLI & In-App launcher):** `batch_transcribe.py --until HH:MM` transcribes (and optionally diarizes) every recording tagged for batch processing, then stops before starting one past the cutoff (for Task Scheduler or on-demand). Can also be launched on-demand from the app via **File > Run Batch Transcription...** (in-app background worker with live status updates or detached process). Recordings are tagged from the recordings list context menu (a peach "Queued" pill marks them), and anything the app declines to transcribe itself is tagged automatically when `general.batch_auto_queue` is on. See [docs/batch-transcription.md](docs/batch-transcription.md).
